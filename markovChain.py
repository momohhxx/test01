...
a=initializeCounterparties(20,np.array([0.25, 0.5, 0.25]))
print(a)
(a==1).sum()
(a==2).sum()
(a==3).sum()
...
mydata=simulateRatingData(100, 30,inventedTransitionMatrix(), np.array([0.25, 0.5, 0.25]))

# mydata=simulateRatingData(20, 4,inventedTransitionMatrix(), np.array([0.25, 0.5, 0.25]))

print('simulated credit state data is: ',mydata.shape)
print(mydata)
  
# below is just to count how many i to j we have in the data;
...
n_ij=getTransitionCount(K=4,T=30,N=100,data=mydata) # result close to 9.47
...
sum(k for k in sum(i for i in n_ij ) ) # result: 2900
...
def estimateCohortTransitionMatrix(K,n_ij,myPeriod):
    ...
    H[-1,:] = np.zeros(K)
    H[-1,-1] = 1        
    # below is nothing but just matrix power, like A^t (t is myPeriod)
    return anp.matrix_power(H,myPeriod)


estimateCohortTransitionMatrix(4,n_ij,myPeriod=1)  # result similar with 9.48
