...
def computeRiskMeasures(M,lossDistribution,alpha):
    ...
    for n in range(0,len(alpha)):
        ...
        #print(f'alpha_n is: {alpha[n]}, alpha_n*(M-1) is: {alpha[n]*(M-1)}')
        
        var[n] = lossDistribution[ np.ceil(alpha[n]*(M-1)).astype(int)]
        
        #print(f'var_n is: {var[n]}')
    return expectedLoss,unExpectedLoss,var,expectedShortfall      

# my note: below analytic...shortfall, paramter alpha is a list of alphas, eg [0.95,0.99]
def analyticExpectedShortfall(N,alpha,pmf,c):
    ...
    for n in range(0,len(alpha)):   
        ...
        # self note: tilda flips all the True values to False, vice versa.
        nanCheck =  ~np.isnan(pmf)
        loss = c*np.interp(myAlpha,cdf[nanCheck],numberDefaults[nanCheck])
        
        # i changed below code on 20251209, seems original 
        # code is wrong. you can't put loss and numberDefaults
        # in the interp(loss,numberDefaults[]...) function. 
        
        # loss = np.interp(myAlpha,cdf[nanCheck],numberDefaults[nanCheck])
        # prob = np.interp(loss,numberDefaults[nanCheck],pmf[nanCheck])
        # loss=[c* i for i in loss]
...
# my notes: below is from formua 3.39;
...
def getBC(N,k):
    ...
    return a/(b*c)
    '''
    # exact=False allows it to handle arrays and returns floats
...
# mynote: gamma density see formula 3.53;
